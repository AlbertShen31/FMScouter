...
6. Open **Role configs** to view and edit each role’s key (×5), green (×3), and blue (×1) attributes. Clicks cycle Off → Key → Green → Blue. Use **Save as new file** to write a named pack under `config/packs/`, then pick that file on Role scores. **Set as defaults** overrides Built-in / Reset with the current weights; **Restore factory** goes back to the Python config.
