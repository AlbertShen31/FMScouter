...
6. Open **Role configs** to view and edit each role’s key (×5), preferred (×3), and useful (×1) attributes. Clicks cycle Off → Key → Preferred → Useful. Use **Positions** chips to assign a role to multiple buckets (Inside Winger is Wide midfielders and Wingers by default). Use **Save as new file** to write a named pack under `config/packs/`, then pick that file on Role scores. **Set as defaults** overrides Built-in / Reset with the current weights; **Restore factory** goes back to the Python config.
