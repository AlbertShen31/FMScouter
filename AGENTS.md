...
- **League limited** when: almost all (≥90%; one leftover probe allowed) minutes-weighted probe `/90` averages (players with minutes only) are **≤ 0.25**, and division has **≥ 300 total minutes**. Sparse leftovers (transfers / continental / one partially filled probe) must **not** veto. Zero-minute rows do not affect aggregates.
- **Player limited**: ≥90 minutes + basics present + almost all probe families zero (total+/90 count as one; one leftover allowed, e.g. Key Passes only) → show “Not tracked”; exclude unavailable metrics from percentile averages.
