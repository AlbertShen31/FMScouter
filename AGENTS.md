...
- **Engine detail level**: MustermannFM / FM Stag cuts live in **Full Detail** space. Default export assumption is **No Detail** (`stats_engine_detail_level` in settings). `scoring/stats_detail_transform.py` maps threshold cuts per metric and `pos_group` for **No Detail** and **Inactive** separately (inactive is a lower tier, not equivalent). Coefficients in `config/stats_detail_levels.json`; refit via `scripts/fit_stats_detail_levels.py`.
