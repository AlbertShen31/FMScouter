...
- **Engine detail tiers (per division)**: MustermannFM / FM Stag cuts live in **Full Detail** space. In Settings, list divisions that export at Full Detail (`stats_full_detail_divisions`). **Limited-tracking** divisions (probe metrics ≈ 0) use **Inactive** transforms; all other divisions default to **No Detail**. `scoring/stats_detail_transform.py` maps threshold cuts per metric and `pos_group` for each tier separately. Coefficients in `config/stats_detail_levels.json`; refit via `scripts/fit_stats_detail_levels.py`. Recompute uploads after changing the full-detail list (cache signature includes it).
